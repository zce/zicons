...
const loadSvg = async path => {
  ...
  const svg = (await readFile(path, 'utf8')).replace(/#0F172A/g, 'currentColor')
  const base64 = Buffer.from(
    svg
      .replace(/currentColor/g, '#64748b')
      .replace('width="24"', 'width="50px"')
      .replace('height="24"', 'height="50px"')
  ).toString('base64')
...
const heroIcons = await Promise.all((await searchSvgs('heroicons/src/24/outline')).map(loadSvg))
